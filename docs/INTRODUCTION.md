...
在进入细节之前, 先搞清楚 SConv 里反复出现的几个核心概念。

### 0.1 什么是"窗口" (window)?

卷积的核心操作是: 把滤波器"贴"在输入的某个位置上, 做元素级乘加。
滤波器每次贴的位置就叫一个**窗口**。
...
对于 stride=1 的卷积, 输出的每个像素对应一个窗口:
- 输出 `[Oh, Ow]` = `[64, 64]` → 有 64×64 = 4096 个窗口
- 每个窗口从输入中取出一个 `[Ic, Fh, Fw]` = `[128, 3, 3]` 的子块

### 0.2 什么是"微内核" (microkernel)?

微内核是整个流水线最内层的一次计算调用。它做的事是:

**一次处理 Nwin 个窗口 × Nf 个滤波器, 算出 Nwin×Nf 个输出值。**

具体来说, 微内核做的是一个矩阵乘:
```
C[Nwin, Nf] += A[K, Nwin]^T × B[K, Nf]
```
其中 K = Ic × Fh × Fw (一次归约的维度)。

### 0.3 所有参数一览

| 符号 | 含义 | 例子值 | 怎么来的 |
|------|------|:---:|---------|
| **Nwin** | 微内核每次处理多少个**窗口** (输出空间位置) | 16 | 用户指定或硬件默认, 取决于 SIMD/SME 寄存器能装多少 |
| **Nf** | 微内核每次处理多少个**滤波器** (输出通道) | 8 | 同上, 与 Nwin 一起决定微内核形状 |
| **Noutput** | 微内核每次输出的标量总数 | 128 | = Nwin × Nf = 16 × 8 |
| **Nc** | 每个通道 tile 包含多少**输入通道** | 32 | CSA 根据 L1 缓存大小算出 |
| **K2** | L2 缓存里复用多少个 tile | 32 (IS) | CSA 根据 L2 大小算出; IS 时是滤波器 tile 数, WS 时是输入 tile 数 |
| **K3** | L3 缓存里复用多少个 tile | 128 (IS) | CSA 根据 L3 大小算出; IS 时是输入 tile 数, WS 时是滤波器 tile 数 |
| **K** | 微内核的归约维度大小 | 288 | = Nc × Fh × Fw = 32 × 3 × 3 |
| **tCH** | 通道分块数 (输入通道切了几段) | 4 | = Ic / Nc = 128 / 32 |
| **in_size** | 一个输入 tile 的字节数 | 576 B | = Nwin × Fh × Fw × 4 (float32) |
| **w_size** | 一个滤波器 tile 的字节数 | 288 B | = Nf × Fh × Fw × 4 |
| **out_size** | 一个输出 tile 的字节数 | 512 B | = Noutput × 4 = 128 × 4 |
| **IS** | 输入驻留调度 | — | 输入 tile 驻留 L1, 滤波器 tile 从 L2 换; 适合输入大、滤波器多的情况 |
| **WS** | 权重驻留调度 | — | 滤波器 tile 驻留 L1, 输入 tile 从 L2 换; 适合滤波器大、输入多的情况 |
| **R_X** | 分块余数 (边界) | 0 | = dimension mod tile_size, 非零则有边界处理 |

### 0.4 参数之间的关系 (一张图)

```
微内核形状 (硬件决定):
  Nwin=16, Nf=8  →  每次 GEMM: C[16,8] += A[288,16]^T × B[288,8]
                                  ↑ K=288

CSA 算出的分块 (缓存决定):
  Nc=32   →  通道维: 每段 32 个通道  (L1 约束)
  K2=32   →  L2 复用 32 个 tile
  K3=128  →  L3 复用 128 个 tile

五层循环 (由内到外):
  Layer 1: Nwin        (微内核窗口维)        16  次/微内核调用
  Layer 2: Nf           (微内核滤波器维)       8  次/微内核调用
  Layer 3: K2 tile      (L2 复用)           32  个 tile/外层迭代
  Layer 4: K3 tile      (L3 复用)          128  个 tile/外层迭代
  Layer 5: Nc           (L1 通道分块)        32  通道/tile
```

### 0.5 IS 和 WS 调度的区别

假设现在有 256 个输入 tile 和 32 个滤波器 tile 要处理:

**IS (输入驻留)**: 输入 tile 驻留 L1, 遍历所有滤波器 tile
```
取一个输入 tile → 放 L1
  取一个滤波器 tile → 放 L2
    微内核调用 (输入不动, 滤波器换)
  下一个滤波器 tile (L2 里换)
...
下一个输入 tile (从 L3 取, L1 换)
```
K2 = L2 能放几个滤波器 tile; K3 = L3 能放几个输入 tile

**WS (权重驻留)**: 滤波器 tile 驻留 L1, 遍历所有输入 tile
```
取一个滤波器 tile → 放 L1
  取一个输入 tile → 放 L2
    微内核调用 (滤波器不动, 输入换)
  下一个输入 tile (L2 里换)
...
下一个滤波器 tile (从 L3 取, L1 换)
```
K2 = L2 能放几个输入 tile; K3 = L3 能放几个滤波器 tile

CSA 的代价模型会算两种调度各自的内存访问量, 选代价小的。
